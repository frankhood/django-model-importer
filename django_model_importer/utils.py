...
class CSVImporter(object):
    ...
    def import_csv(self, file_path, **kwargs):
        ...
        for row in CsvUnicodeReader(file_path,  # open(file_path, 'rb'),
            ...
            if row_number >= from_row:
                if self._import_row(row, row_number, **kwargs):
                    ...
                    error_number += 1
        self.logger.info(_("Imported CSV of {0} rows of which {1} were not processed".format(row_number, error_number)))
    ...
    def _import_row(self, row, row_number, **kwargs):
        if len(row) > self.get_expected_cols():
            self.logger.info("Numero colonne : %s invece di %s" % (len(row), self.get_expected_cols()))
