...
def coverage(c):
    ...
    """
    c.run("coverage run --source django-model-importer runtests.py tests")
...
def docs(c):
    ...
    """
    c.run("rm -f docs/django-model-importer.rst")
    c.run("rm -f docs/modules.rst")
    c.run("sphinx-apidoc -o docs/ django_model_importer")
...
def lint(c):
    ...
    """
    c.run("flake8 django-model-importer tests")
...
def release(c, bumpsize=''):
    ...
    c.run('git tag -a {version} -m "New version: {version}"'.format(version=django_model_importer.__version__))
    c.run("git push --tags")
