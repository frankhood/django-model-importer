...
from tests.example import factories
...
class PollImportUnitTest(TestCase):
    def test_poll_import(self):
        first_poll_category = factories.PollCategoryFactory(name="First")
        second_poll_category = factories.PollCategoryFactory(name="Second")
        poll = factories.PollFactory(
            title="Test Title",
            user=factories.UserFactory(username="pippo"),
            poll_categories=[first_poll_category, second_poll_category],
        )
        factories.QuestionFactory(text="Why you are coding?", poll=poll)
        factories.QuestionFactory(text="Why you are testing?", poll=poll)
        ...
        self.assertTrue(Poll.objects.get(title="New Title"))
        User = apps.get_model(settings.AUTH_USER_MODEL)
        self.assertTrue(User.objects.get(username="pluto"))
        self.assertTrue(User.objects.get(username="paperino"))
        self.assertTrue(Question.objects.get(text="Why you are writing?"))
